...
const handleDuplicatedFieldsDB = (err) => {
  ...
  const message = `Campos duplicados ${value}, por favor use otro nombre`;
  return new AppError(message, 400);
};

const handleValidationErrorDB = (err) => {
  const errors = Object.values(err.erros).map((el) => el.message);
  const message = `Campo invalido ${errors.join('. ')}`;
  return new AppError(message, 400);
};
...
const handleJWTExpiredError = () => {
  return new AppError('Token Expirado, por favor inicia sesion de nuevo', 401);
};
